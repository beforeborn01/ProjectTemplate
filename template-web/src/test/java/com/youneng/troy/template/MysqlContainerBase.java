...
public class MysqlContainerBase implements BeforeAllCallback {
    ...
    public void beforeAll(ExtensionContext context) {
        if (mysql == null) {
            mysql = (MySQLContainer) new MySQLContainer(DockerImageName.parse("mysql:5.7")).withDatabaseName("template").withInitScript("table.sql") // 指定数据库初始化脚本，默认的db为test
...
//            System.setProperty("jdbc.url", mysql.getJdbcUrl());// 使用容器对象获取jdbcUrl，会自动设置合适的ip和端口号
            System.setProperty("tidb.host", "localhost");
            System.setProperty("tidb.port", mysql.getFirstMappedPort() + "");
